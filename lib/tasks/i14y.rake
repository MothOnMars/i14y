namespace :i14y do
  ...
  task :reindex, [:entity_name] => [:environment] do |_t, args|
    ...
    aliases = DEFAULT_CLIENT.indices.get_alias(name: wildcard)
    aliases.each do |old_es_index_name, alias_names|
      ...
      host_hash = DEFAULT_CLIENT.transport.hosts.first
      ...
      DEFAULT_CLIENT.indices.delete(index: old_es_index_name)
    end
  end
  ...
  task clear_all: :environment do
    Dir[Rails.root.join('app', 'templates', '*.rb')].each do |template_generator|
      entity_name = File.basename(template_generator, '.rb')
      DEFAULT_CLIENT.indices.delete_template(name: entity_name) rescue Elasticsearch::Transport::Transport::Errors::NotFound
    end
